...
void AppTask::FunctionHandler(AppEvent *aEvent) {
    if (aEvent->Type == AppEvent::kEventType_Button) {
        if (aEvent->ButtonEvent.ButtonIdx == BUTTON_USER1) {
            if (xTimerIsTimerActive(sPushButtonTimeoutTimer) == 0) {
                xTimerStart(sPushButtonTimeoutTimer, 0);
                NvmTimerCpt = 0;
                NvmButtonStateCpt = 0;
            }
        } else if (aEvent->ButtonEvent.ButtonIdx == BUTTON_USER2) {
            if (sHaveFabric == true) { // only use if a fabric is avalaible
...
#if ((OTA_SUPPORT == 1) && (CFG_LCD_SUPPORTED == 1))
